...
def home(request):
    ...
    pets=Petstore.objects.all().reverse()
    length=len(pets)
...
# cretae json file of database
def CreateJson(request):
    query=Petstore.objects.all()
    data=serialize('json',query)
    loc='static/json/jdata.json'
    ...
    jdata=json.dumps(jsonn,indent=4)

    with open(loc,'w')as file:
        file.write(jdata)

    # return JsonResponse(data,safe=False)
    return HttpResponse("Json file ban kar tyaar ho gyi hai")
# -----------------------------------------------------------------------------------------
